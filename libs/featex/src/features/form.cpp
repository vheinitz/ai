...
QString FormInfo ="Ratio of the area of a shape to the area of a circle having the same perimeter";
REGISTER_FEATURE( Form, FeatureForm, FormInfo );
...
QString FeatureForm::version()
{
	return "0.0.1";
}

TPFeatureResults FeatureForm::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
...
double extractObjectFeature( cv::Mat obj, int metric )
{
	Mat cell =  obj.clone() ;
	//Mat cellDbgImg( 150, 100, CV_8UC1 );
	//cellDbgImg=0;
