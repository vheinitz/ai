...
QImage I2Q(IplImage *iplImg); //todo C&P in imageprocessing
...
QString SpecialANCAInfo ="Pseudo Feature for extracting ANCA-trimmed feature vector";
REGISTER_FEATURE( SpecialANCA, FeatureSpecialANCA, SpecialANCAInfo );

FeatureSpecialANCA::FeatureSpecialANCA()
{	
}

FeatureSpecialANCA::~FeatureSpecialANCA(){}

QString FeatureSpecialANCA::version()
{
	return "0.0.1";
}

TPFeatureResults FeatureSpecialANCA::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	return TPFeatureResults(new FeatureResults(-1));
}
...
TPFeatureResults FeatureLocalMax::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	...
	QElapsedTimer timer;
	timer.start();
	res->_value = 0;//TODO ImageProcessing::findLocMaxima( img, params["step"].toInt(), params["minThreshold"].toInt(), &positions );
...
TPFeatureResults FeatureImgParam::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeaturePeaks::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
//////////
//////////////////////
QString LayersRatioInfo ="Ratio of area in cell-base and highest level";
REGISTER_FEATURE( LayersRatio, FeatureLayersRatio, LayersRatioInfo );

FeatureLayersRatio::FeatureLayersRatio()
{
	ParamInfo pi;
...
QString FeatureLayersRatio::version()
{
	return "0.0.6";
}

TPFeatureResults FeatureLayersRatio::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;	
	...
	foreach( cv::RotatedRect rr,  cells )
	{
		...
		if(x>0 && y>0 && x+w < om.cols && y+h < om.rows)
		{			
			
			cv::Mat roi = om(roirect);	
			cv::Mat cell;
			
			cv::Mat lbase, lhigh;
			normalize(roi,cell,0,5,CV_MINMAX);

			int hl=5;
			int nz=0;
			for (int i=5; i>=0; i--)
			{				
				cv::threshold(cell ,lhigh,i, 255,  CV_THRESH_BINARY);
				hl=i;
				nz = countNonZero(lhigh);
				if (nz)
					break;
			}
			if (nz < 1 || nz > 30)
			...
			cellsNum++;			
			cv::threshold(cell ,lbase,1, 255,  CV_THRESH_BINARY);
			int nzb = countNonZero(lbase);
				if (nzb <=0) nzb=1; //prevent div 0 crash
				double r = (double)nz / nzb ;
			ratiosum += r;			
			/*if ( cnt++ < 10 )
			{
				imshow( "test", roi);
			...
			positions.append(QRect(x, y, w, h ));
		}
	}

	res->_value = cellsNum > 0 ? ratiosum/cellsNum : 0;
...
QString MembraneMeanRatioInfo ="Ratio of mean of membrane to rest of the cell";
REGISTER_FEATURE( MembraneMeanRatio, FeatureMembraneMeanRatio, MembraneMeanRatioInfo );

FeatureMembraneMeanRatio::FeatureMembraneMeanRatio()
{
	ParamInfo pi;
...
QString FeatureMembraneMeanRatio::version()
{
	return "0.0.6";
}

TPFeatureResults FeatureMembraneMeanRatio::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;	
	...
	int pyrFactor = 2; //TOODO optimize,get scaled image from cache
	...
	foreach( cv::RotatedRect rr,  cells )
	{
		...
		{			
			...
			cv::Mat cellmask;
			double cmean = cv::mean( roi )[0];

			cv::threshold(roi ,cellmask, cmean, 255,  CV_THRESH_BINARY);
			...
			Mat kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE,
							  ...
							  cv::Point(px, px) );
			Mat coremask;
			erode(cellmask,coremask,kernel);
			erode(coremask,coremask,kernel);
			
			Mat ringmask = cellmask ^ coremask;

			double ringmean = cv::mean( roi & ringmask )[0];
			double coremean = cv::mean( roi & coremask )[0];

	
			//imshow( "coremask", coremask);
			//imshow( "ringmask", ringmask);
			
			mmr += coremean > 0 ? ringmean/coremean : 0;
			positions.append(QRect(x, y, w, h ));
		}
	}

	res->_value = cells.size() > 0 ? mmr/cells.size() : 0;
...
//////////////////////
QString CellPeaksInfo ="Intensity peaks within cells";
REGISTER_FEATURE( CellPeaks, FeatureCellPeaks, CellPeaksInfo );

FeatureCellPeaks::FeatureCellPeaks()
{
	ParamInfo pi( "pmin", "min peak diameter", "1-max" );
	_paramInfos.append(pi);

	pi._name = "pmax" ;
	pi._info = "max peak diameter", "min-200" ;
	_paramInfos.append(pi);
...
QString FeatureCellPeaks::version()
{
	return "0.0.2";
}

TPFeatureResults FeatureCellPeaks::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;

	int pmin, pmax;
	if (!params.contains("pmin"))
	{
		//todo error
		return res;
	}
	if (!params.contains("pmax"))
	{
		//todo error
		return res;
	}

	pmin = params["pmin"].toInt();
	pmax = params["pmax"].toInt();
	...
	int pyrFactor = 1; //TOODO optimize,get scaled image from cache
...
TPFeatureResults FeaturePeaksPerLine::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureBlobs::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureNdnaIndex::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{	
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureNumOfStroks::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureSpinsPerLine::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureMatchHistogram::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureSequence::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureHuMean::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
	...
	if (hunum>=0 && hunum <= 6)
	{
		...
		ce.setMaxCell(max/pyrFactor);
		CeMx.lock();
		TRotatedRectList cells = ce.findCellRects( m, small.text( "hashkey" ) );
		CeMx.unlock();
		foreach( cv::RotatedRect rr,  cells )
		{
			cv::Rect cr = rr.boundingRect();
...
TPFeatureResults FeatureCellMetrics::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
	...
	CeMx.lock();
	TRotatedRectList cells = ce.findCellRects( m, hk, minNum, similar, roi );
...
TPFeatureResults FeatureExif::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureCellVariance::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureCellTexture::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureCellShape::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
...
TPFeatureResults FeatureFindTempl::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
	...
	QImage tmpl(QString( "c:/tmp/tmpl/%1.png" ).arg( tmplFn ) );
...
//////////////////////////
QString PosNDnaInfo ="Positive Ndna Probability";
REGISTER_FEATURE( PosNDna, FeaturePosNDna, PosNDnaInfo );

FeaturePosNDna::FeaturePosNDna()
{
	ParamInfo pi( "minCell" );
	_paramInfos.append(pi);
...
QString FeaturePosNDna::version()
{
	return "0.0.1";
}

TPFeatureResults FeaturePosNDna::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	TPFeatureResults res(new FeatureResults(-1));
	QList<QRect> positions;
	...
	int param = params["param"].toInt();
	int fidx = params["fidx"].toInt();
	ce.setMinCell(min/pyrFactor);
	ce.setMaxCell(max/pyrFactor);
	CeMx.lock();	
	TRotatedRectList cells = ce.findCellRects( m );
	cv::Mat normCells = m.clone();
	//imshow("NormCellsImage", ce.cellsNormImage() );

	int x=0,y=0;
	for( int i = 0; i< cells.size(); i++ )
	{
		 Mat norm = ce.normalizedCellAt( 
			 cells.at(i).center, 
			 cv::Size(32,32) );		 

		 if ( norm.rows <=0 || norm.cols <=0 )
			 continue;
		 //Mat c = ce.cellAt( 
		//	 cells.at(i).center );	
		 norm.copyTo( normCells( cv::Rect(x,y,32,32) ) );
		 //imshow("NormCell", norm);

		 x+=40;//c.cols;
		 if ( x >= normCells.cols-100 )
		 {
			x = 0; 
			y+=40;
		 }
	...
	//imshow("NormCells", normCells);

	const std::vector< std::vector<cv::Point> > & contours = ce.cellContours();

	CeMx.unlock();

	///
	typedef std::vector<cv::Point> TContour;
	Mat drawing = m.clone();//Mat::zeros( mo.size(), CV_8UC3 );
	vector<Vec4i> hierarchy;
	
	std::vector< std::vector<cv::Point> > userContours;
    ...
    for( int i = 0; i< contours.size(); i++ )
     {
	   ...
	   TContour tmp;
	   TContour chp;
	   cv::convexHull(contours.at(i),tmp);
	   chp = tmp;
	   //approxPolyDP(Mat(tmp), chp, arcLength(Mat(tmp), true) * 0.05, true);
	   if ( chp.size() < 9 ) continue;
	   
	   userContours.push_back(chp);
	   cv::Point opositeGeisel = chp.at(0);
	   chp.push_back( chp.at(0) );

	   int GeiselCnt=0;
	   int GeiselIdx=0;
	   for( int pi = 0; pi< chp.size()-2; pi++ )
	   {		   
		   cv::Point p1 = chp.at(pi);
		   cv::Point p2 = chp.at(pi+1);
		   cv::Point p3 = chp.at(pi+2);	 
		   cv::Point pv1 = p2-p1;
		   cv::Point pv2 = p2-p3;
		   double angle=0.0; 
		   angle = atan2(double(pv2.y), double(pv2.x)) - atan2( double(pv1.y), double(pv1.x) );
		   ...
		   if (angle < 120)
		   {
			   if (GeiselCnt ++ > 1) {
				...
				positions.append(QRect(p2.x*pyrFactor, p2.y*pyrFactor, 3,3 ));
				break;
		   }
	   }
		  if ( GeiselCnt )
		  {
			int mid = chp.size()/2;
			if ( GeiselIdx + mid > chp.size()  )
			{
				opositeGeisel = chp.at( GeiselIdx +1 - mid );
			}
			else
			{
				opositeGeisel = chp.at( GeiselIdx -1 + mid );
			}
				circle( drawing, chp.at( GeiselIdx ), 3,  Scalar(255), 2, 8, 0 );
				circle( drawing, opositeGeisel, 6,  Scalar(255), 2, 8, 0 );
		  }       
     }
		
	for( int i = 0; i< userContours.size(); i++ )
	{
		drawContours( drawing, userContours, i, Scalar(  100 ), 1, 1, hierarchy, 0, Point() );
	}
...
//////////////////////////
QString PeakParamsInfo =""
"Algorithm: Find 3 maxima subsequently by masking each previos maximum.\n"
" Calculate these statistics depending on param: (M[1-3] - maxima value, \n"
" P[1-3] - maxima positions, C- cell center, Avg - cell average), dist(X,Y) - distance\n"
"  0: M1/Avg; 1: dist(P1,C), 2: M2/Avg; 3: dist(P2,C); 4: M3/avg; 5: dist(P3,C); \n"
"  6: dist(P1,P2); 7: dist(P2,P3); 8: dist(P1,P3); 9: M2/M1; 10: M3/M2; 11: M3/M1 ";
REGISTER_FEATURE( PeakParams, FeaturePeakParams, PeakParamsInfo );

FeaturePeakParams::FeaturePeakParams()
{
	ParamInfo pi( "minCell" );
	_paramInfos.append(pi);
...
QString FeaturePeakParams::version()
{
	return "0.0.1";
}

static QMap<QString, QList<double> > fh;

TPFeatureResults FeaturePeakParams::extract( QImage & img, TFeatParams params, QList<QRect> * rois )
{
	QString hash = QString(QCryptographicHash::hash( QByteArray((char*)img.bits(), img.byteCount() ) ,QCryptographicHash::Md5).toHex());
	...
	int x=0,y=0;
	vector <double> max1_by_avg;
	vector <double> max2_by_avg;
	vector <double> max3_by_avg;
	vector <double> dists1;
	vector <double> dists2;
	vector <double> dists3;
	vector <double> dists1_2;
	vector <double> dists1_3;
	vector <double> dists2_3;
	vector <double> max2_by_max1;
	vector <double> max3_by_max1;
	vector <double> max3_by_max2;
	QMultiMap<double, cv::RotatedRect> similarSizeCells;
	QMultiMap<double, cv::RotatedRect> similarValueCells;

	foreach( cv::RotatedRect rr,  cells )
	{
		similarSizeCells.insert( rr.boundingRect().area(), rr );				
	}

	int from = similarSizeCells.size() / 4;
	int to = similarSizeCells.size() / 4 + similarSizeCells.size() / 2;
	int i=0;

	foreach( cv::RotatedRect rr, similarSizeCells.values() )
	{
		i++;
		if (i<from || i>to) 
			continue;	
		cv::Rect roi = rr.boundingRect();
		similarValueCells.insert( cv::mean(m(roi))[0] , rr );
	}

	from = similarValueCells.size() / 4;
	to = similarValueCells.size() / 4 + similarValueCells.size() / 2;
	i=0;
	
	/// TEST
	//cv::imwrite( QString( "c:/temp/pd_img_log/%1.png" ).arg(hash).toStdString(), m );
	//Mat m_ann = m.clone();
	/// TEST

	foreach( cv::RotatedRect rr, similarValueCells.values() )
	{
		i++;
		if (i<from || i>to) 
			continue;		
		...
		//cv::rectangle( m_ann,cr,cv::Scalar(255),1 );/// TEST

		positions.append(QRect(cr.x*pyrFactor, cr.y*pyrFactor, cr.width*pyrFactor, cr.height*pyrFactor ));
		Mat c =  m( roi );
		double min, max1, max2, max3;
		cv::Point min_loc, max_loc1, max_loc2,max_loc3;
		cv::minMaxLoc(c, &min, &max1, &min_loc, &max_loc1);
		max1_by_avg.push_back( max1 / cv::mean(c)[0] );
		cv::Point center = rr.center;
		dists1.push_back( abs(cv::norm(( center-cv::Point(roi.x, roi.y) )-max_loc1)) ); 

		circle( c, max_loc1, 5,  Scalar(0), -1, 0, 0 );
		cv::minMaxLoc(c, &min, &max2, &min_loc, &max_loc2);

		//circle( m_ann, max_loc1 + cv::Point(cr.x,cr.y) , 5,  Scalar(220), 1, 0, 0 ); ///TEST
		//circle( m_ann, max_loc2 + cv::Point(cr.x,cr.y) , 3,  Scalar(180), 1, 0, 0 ); ///TEST
		//circle( m_ann, center  , 2,  Scalar(255), 1, 0, 0 ); ///TEST

		max2_by_avg.push_back( max2 / cv::mean(c)[0] );		
		dists2.push_back( abs(cv::norm(( center-cv::Point(roi.x, roi.y) )-max_loc2)) ); 

		dists1_2.push_back( abs(cv::norm(max_loc1-max_loc2)) );
		max2_by_max1.push_back( max2/max1 );		

		///
		circle( c, max_loc2, 5,  Scalar(0), -1, 0, 0 );
		cv::minMaxLoc(c, &min, &max3, &min_loc, &max_loc3);

		max3_by_avg.push_back( max3 / cv::mean(c)[0] );		
		dists3.push_back( abs(cv::norm(( center-cv::Point(roi.x, roi.y) )-max_loc3)) ); 

		dists1_3.push_back( abs(cv::norm(max_loc1-max_loc3)) );
		max3_by_max1.push_back( max3/max1 );		

		dists2_3.push_back( abs(cv::norm(max_loc2-max_loc3)) );
		max3_by_max2.push_back( max3/max2 );		


	}	

	/// TEST
	//cv::imwrite( QString( "c:/temp/pd_img_log/%1_ann.png" ).arg(hash).toStdString(), m_ann );
	/// TEST	

	fh[hash]	<<	cv::mean( Mat( max1_by_avg ) )[0]
				<<	cv::mean( Mat( dists1 ) )[0]
				<<  cv::mean( Mat( max2_by_avg ) )[0]
				<<  cv::mean( Mat( dists2 ) )[0]
				<<  cv::mean( Mat( max3_by_avg ) )[0]
				<<  cv::mean( Mat( dists3 ) )[0]
				<<  cv::mean( Mat( dists1_2 ) )[0]
				<<  cv::mean( Mat( dists2_3 ) )[0]
				<<  cv::mean( Mat( dists1_3 ) )[0]
				<<  cv::mean( Mat( max2_by_max1 ) )[0]
				<<  cv::mean( Mat( max3_by_max2 ) )[0]				
				<<  cv::mean( Mat( max3_by_max1 ) )[0];
